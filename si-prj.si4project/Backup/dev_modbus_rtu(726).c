    ...
    while (1)
    {
		...
		if (err == RT_EOK)		
		{
			...
			*str_val = '\0';
			...
			if (str_val)
			{
				extern void mqtt_service_reply_pub(const char *topic_fliter, const char *id, const char *code, const char *data);
				...
				else if (!rt_strstr(str_cmd, DEV_CTRL_CMD))
				{
					...
					if (str_val)
					{
						if (!rt_strcasecmp(str_val, DEV_CTRL_CMD_REBOOT))
						{
							...
							rt_thread_mdelay(rt_tick_from_millisecond(10 * RT_TICK_PER_SECOND));
						...
						else if (!rt_strcasecmp(str_val, DEV_CTRL_CMD_BEEP))
						{
							...
							extern rt_err_t beep_ctrl_song(rt_uint32_t cmd_val);
							beep_ctrl_song(1);
						}
